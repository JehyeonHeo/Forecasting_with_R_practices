...
str(gasoline)
head(gasoline)
# It is weekly data and it would be useful to make model with harmonic regression.
...
# as more pairs used, more the fitted line looked more like the original data. But the fitted lines didn't follow the trend well.
...
for(num in 1:26){
  ...
  # If both of the minimum values are found, the loop don't need to run anymore. Therefore print the result number of pairs and break the loop.
  # If num = 1, don't run below codes and move to num = 2. With just the result of num = 1, I cannot know whether the AICc or CV is minimum.
  if(num != 1){
...
str(huron)
head(huron)
# There are lots of fluctuations that show some cyclic behaviour. And upto about 1930 there were downward trend, but after that, the trend disappeared.
...
# It looked like spline model didn't catch the trend well. Linear model and log transformed linear model couldn't reflect the trend change around 1915.
...
# Linear regression model shows that the point forecasts and the prediction intervals decrease as time goes on. It didn't reflect the trend change around 1915.
# Piecewise linear regression model shows that the point forecasts and the prediction intervals are almost same even if time goes on. It reflected the trend change.
