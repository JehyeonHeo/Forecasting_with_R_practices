...
# The forecasts were similar to real data.

# make a function to automatically fit models to given data and output accuracy using test set for each model.
...
# The forecasts were similar to real data in about an year. But real data showed slightly more increase than the forecasts from that time. The best model could follow the general trend, but a little short for predicting more fastly increasing trend.
...
# I think that ETS(M, N, N) is not good. The price almost went to near 0 and no one knows whether it will go up or maintain or go down more. The model didn't yield helpful forecasting.
...
# I think that ETS(A, N, A) is good. It reflected the already existing seasonality.
...
# It looked like ets function couldn't find well fitted ETS model when there were aperiodic fluctuations(or cycles) in the data. In such cases, ets function couldn't find trend or seasonality and just yields naive forecasts.
