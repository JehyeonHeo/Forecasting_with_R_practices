...
autoplot(labour, series="Data") +
  ...
  scale_colour_manual(values=c("gray","blue","red"),
                     breaks=c("Data","Seasonally Adjusted","Trend"))
# Overall, the number of people in the civilian labour force in Australia increased over time. There were big recession around 1991 and 1992, and seasonally adjusted data followed the event. From ggsubseriesplot, I could find that there were relatively small changes for seasonal component over time compared to the original data scale. 

# b. Is the recession of 1991/1992 visible in the estimated components?
# It is seen in the remainder component.
```
...
ggseasonplot(cangas)
# The gas production increased in winter and decreased in summer. Maybe the cold weather in winter increased the demand for the gas that the production increased. But the production increased in summer as time went on. Maybe the demand for electricity to run air conditioner increased over time caused the situation. 
...
# seas function did multiplicative decomposition. Therefore seasonal component and remainder component have mean at 1, not 0. And the proportion of seasonality to trend decreased, increased and again decreased. 
# From the plots, I could see that the seasonally adjusted data of STL decomposition is rougher than the ones of the other methods. It can mean that STL method is more robust and unusual observations affect the remainder component more when using STL method.
...
# can see from the plot that the forecasts from stlf function are more similar to the original data than the forecasts from snaive function. stlf can use trend too and its seasonality can change over time, which can predict near future better.
